...
#include "MAX30105.h"
#include "hardware/i2c.h" // Para funções I2C do Pico SDK
#include "pico/stdlib.h"  // Para sleep_ms(), etc.
#include "pico/time.h"    // Para to_ms_since_boot()
#include <string.h>       // Para memcpy
#include <stdio.h>        // Para printf (para depuração, pode remover depois)
...
MAX30105::MAX30105() {
  // Constructor
  _i2cPort = i2c_default; // Inicializa com a instância I2C padrão
  _i2caddr = MAX30105_ADDRESS;
}

// Modificado: Função begin para usar i2c_inst_t* e sem configuração de clock
bool MAX30105::begin(i2c_inst_t *wirePort, uint8_t i2caddr) {

  _i2cPort = wirePort; // Recebe a instância I2C do usuário
  _i2caddr = i2caddr;

  // A inicialização real do I2C (i2c_init, gpio_set_function) deve ser feita
  // no seu main.c antes de chamar este begin().
...
void MAX30105::softReset(void) {
  ...
  // Timeout after 100ms
  uint32_t startTime = to_ms_since_boot(get_absolute_time()); // MODIFICADO: millis()
  while (to_ms_since_boot(get_absolute_time()) - startTime < 100) // MODIFICADO: millis()
  {
    uint8_t response = readRegister8(_i2caddr, MAX30105_MODECONFIG);
    if ((response & MAX30105_RESET) == 0) break; //We're done!
    sleep_ms(1); // MODIFICADO: delay(1)
  }
}
...
void MAX30105::setup(uint8_t powerLevel, uint8_t sampleAverage, uint8_t ledMode, int sampleRate, int pulseWidth, int adcRange) { // MODIFICADO: 'byte' por 'uint8_t'
...
uint16_t MAX30105::check(void)
{
  ...
  uint8_t readPointer = getReadPointer(); // MODIFICADO: 'byte' por 'uint8_t'
  uint8_t writePointer = getWritePointer(); // MODIFICADO: 'byte' por 'uint8_t'
  ...
  if (readPointer != writePointer)
  {
    ...
    // _i2cPort->beginTransmission(MAX30105_ADDRESS); // REMOVIDO
    // _i2cPort->write(MAX30105_FIFODATA);          // REMOVIDO
    // _i2cPort->endTransmission();                 // REMOVIDO

    // SUBSTITUIDO PELO I2C DO PICO SDK
    uint8_t reg_addr = MAX30105_FIFODATA;
    // Envia o endereço do registrador de onde queremos ler.
    // O 'true' indica que é um "restart" (não envia condição STOP), preparando para a leitura.
    i2c_write_blocking(_i2cPort, MAX30105_ADDRESS, &reg_addr, 1, true);

    // Buffer para armazenar os dados lidos. O tamanho máximo do FIFO é 32 amostras,
    // cada amostra pode ter até 3 LEDs * 3 bytes/LED = 9 bytes.
    // 32 amostras * 9 bytes/amostra = 288 bytes.
    // Precisamos de um buffer para o pior caso.
    uint8_t rawData[bytesLeftToRead]; // Alocado dinamicamente no stack (OK para tamanhos pequenos)

    // Lê os bytes restantes
    // O 'false' indica que envia condição STOP após a leitura.
    i2c_read_blocking(_i2cPort, MAX30105_ADDRESS, rawData, bytesLeftToRead, false);

    int rawDataIndex = 0; // Índice para o buffer rawData

    // Agora, processar os dados lidos
    while (bytesLeftToRead > 0)
    {
      ...
      // Os dados já foram lidos para 'rawData'. Apenas copie.
      temp[3] = 0; // Assume 24-bit data, pad with 0 for 32-bit uint
      temp[2] = rawData[rawDataIndex++];
      temp[1] = rawData[rawDataIndex++];
      temp[0] = rawData[rawDataIndex++];

      //Convert array to long
      memcpy(&tempLong, temp, sizeof(tempLong));
    
      tempLong &= 0x3FFFF; //Zero out all but 18 bits (MAX30105 output resolution)
...
{
  uint32_t markTime = to_ms_since_boot(get_absolute_time()); // MODIFICADO: millis()
  
  while(1)
  {
    if(to_ms_since_boot(get_absolute_time()) - markTime > maxTimeToCheck) return(false); // MODIFICADO: millis()

    if(check() > 0) //We found new data! (check() retorna numberOfSamples, que pode ser 0 ou > 0)
      return(true);
...
uint8_t MAX30105::readRegister8(uint8_t address, uint8_t reg) {
  ...
  // O 'true' no final indica que não enviamos um STOP após a escrita, mantendo a conexão para uma leitura "restart".
  int ret = i2c_write_blocking(_i2cPort, address, tx_buf, 1, true); 
  if (ret != PICO_ERROR_GENERIC && ret != PICO_ERROR_TIMEOUT) { // Verifica se a escrita foi bem-sucedida
      // Lê 1 byte do registrador
      // O 'false' no final indica que envia um STOP após a leitura.
      ret = i2c_read_blocking(_i2cPort, address, rx_buf, 1, false);
      if (ret != PICO_ERROR_GENERIC && ret != PICO_ERROR_TIMEOUT) { // Verifica se a leitura foi bem-sucedida
          return rx_buf[0];
      }
  }
  printf("I2C read failed for reg 0x%02X, addr 0x%02X\n", reg, address); // Para depuração
  return (0); // Fail
}

void MAX30105::writeRegister8(uint8_t address, uint8_t reg, uint8_t value) {
  uint8_t tx_buf[2] = {reg, value};
  // Escreve o registrador e o valor.
  // O 'false' no final indica que envia um STOP após a escrita.
  int ret = i2c_write_blocking(_i2cPort, address, tx_buf, 2, false);
  if (ret == PICO_ERROR_GENERIC || ret == PICO_ERROR_TIMEOUT) {
      printf("I2C write failed for reg 0x%02X, addr 0x%02X, val 0x%02X\n", reg, address, value); // Para depuração
  }
}