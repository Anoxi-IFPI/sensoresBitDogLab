...
void atualizar_display_por_lux(float lux) {
    ...
    if (lux < 10) {
        ...
        snprintf(mensagem, sizeof(mensagem), "Pouca luz: %.1f lx", lux);
        ...
        snprintf(mensagem, sizeof(mensagem), "Bem iluminado: %.1f lx", lux);
        pwm_set_gpio_level(VERDE, periodo);
        pwm_set_gpio_level(AZUL, periodo * 0.5);
        pwm_set_gpio_level(VERMELHO, periodo * 0.2);
    } 
    else {
        snprintf(mensagem, sizeof(mensagem), "Muito claro: %.1f lx", lux);
        pwm_set_gpio_level(VERDE, periodo);
